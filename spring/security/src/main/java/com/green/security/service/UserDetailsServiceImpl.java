...
public class UserDetailsServiceImpl implements UserDetailsService {
  ...
  // AuthenticationManager 객체로 로그인 검증을 시작하면 실행하는 매서드
  // 해당 매서드에서는
  // 1. DB에서 로그인하려는 정보를 조회한다
  // 2.
  @Override
  public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
    System.out.println("UserDetailsServiceImpl 클래스 안의 loadUserByUsername 매서드 실행");
    // DB에서 로그인 하려는 회원의 정보를 조회(이메일, 비번, 권한)
    ...
    // DB에서 조회한 로그인 정보를 리턴한다
    // 이렇게 리턴된 로긍니 정보는 AuthenticationManager한테 전달한다
    // 다만, 조회한 데이터(dto)를 그대로 리턴하면 안된다.
    // Security에서 제공하는 로그인한 정보를 저장할 수 있도록 만든 UserDetails 인터페이스 자료형에,
    // 로그인 하려는 회원 정보를 담아서 리턴해야한다.
