...
# EMP 테이블의 사원 중에서 급여가 300이상 700이하인 사원들의 사번, 사원명, 급여를 조회
...
# EMP 테이블에서 급여가 400이거나 600이건  800인 사원들의 모든 정보를 조회 (IN)
SELECT *
FROM emp
...
# emp 에이블의 모든 정보를 급여기준 내림차순 정렬 (급여 높은 순부터)
# 오름차순 : ASC(생략가능), 내림차순 : DESC
SELECT *
FROM emp
ORDER BY ;

# 직급이 사원, 대리, 과장인 사원 중에서 급여가 200~700사이 사원들의 사번, 사원명, 직급, 급여를 조회하라
# 사원명 기준 오름차순 정렬
# 정렬절(ORDER BY)는 제일 마지막에~
SELECT EMPNO, ENAME, JOB, SAL
FROM emp
WHERE JOB IN('사원', '대리', '과장')
AND (SAL >= 200 AND SAL <= 700)
ORDER BY ENAME ASC;


# 사원들의 사번, 사원명, 부서번호, 급여 정보 조회
# 부서 번호 기준 오름차순 정렬 후 급여기준 내림차순 정렬
# 정렬 여러개면 쉼표(,)로 연결하기
...
# 와일드카드 : '%', '_'
# '_' : 정해지지 않은 한글자
# '%' : 정해지지 않은 글자 (글자수 제한 없음)
# ex) '김__' : 김으로 시작하는 세글자.
#     '김%' : 김으로 시작하는 모든 글자  -> 김, 김가, 김가나, 김가나다, ....EMPNO.
#     '_이%' : 두번째 글자가 '이'인 모든 글자
# 사원명에 '이'가 포함된 사원들의 모든 정보 조회
...
#난이도 중
#카테고리가 **'IT'**이거나 **'소설'**인 도서 중에서, 가격이 20,000원 이상인 도서의 제목, 저자, 가격을 조회하시오.
SELECT TITLE, AUTHOR, PRICE
FROM books
WHERE (CATEGORY LIKE '%IT%' OR '%소설%') AND (PRICE >=20000);

#도서 제목에 **'데이터'**라는 단어가 포함된 모든 도서의 정보를 조회하되, **가격이 높은 순(내림차순)**으로 정렬하시오.
SELECT *
FROM books
...
#**재고 수량(STOCK)**이 NULL이거나 0인 도서의 제목과 카테고리를 조회하시오.
...
#2022년 1월 1일부터 2023년 12월 31일 사이에 출판된 도서의 제목과 출판일을 조회하시오. (BETWEEN 활용)
...
#저자의 성이 **'이'**씨인 도서들을 조회하여 제목 기준 오름차순으로 정렬하시오. (별칭을 활용하여 제목은 '도서명', 저자는 '글쓴이'로 표시)
SELECT TITLE AS '도서명', CATEGORY AS '글쓴이'
FROM books
WHERE AUTHOR LIKE '이%'
ORDER BY TITLE ASC;
...
#난이도 상
#'IT' 카테고리 도서 중 가격이 30,000원 이상이거나, '소설' 카테고리 중 가격이 15,000원 이하인 도서의 모든 컬럼을 조회하시오.
SELECT *
FROM books
...
#재고(STOCK)가 10권 이상인 도서들 중에서, 제목이 **'혼자'**로 시작하지 않는 도서들의 정보를 조회하시오.
SELECT *
FROM books
WHERE (STOCK >= 10) AND (TITLE NOT LIKE '혼자%');




#저자가 **'이데이터'**이거나 **'김철수'**인 도서 중, 2021년 이후에 출판된 도서의 제목, 저자, 출판일을 조회하시오. (단, 출판일 기준 내림차순 정렬)
...
#가격이 15,000원 이상 30,000원 이하인 도서 중, 재고가 NULL이 아닌 도서의 제목, 가격, 재고를 조회하시오.
...
#제목에 '공부' 또는 **'설계'**가 포함된 도서 중 카테고리가 **'IT'**인 도서의 제목, 카테고리, 가격을 조회하시오.
#단, 가격에 10% 부가세를 더한 값을 **'총합계'**라는 별칭으로 계산하여 함께 보여주시오. (힌트: PRICE * 1.1)

SELECT TITLE, CATEGORY, PRICE, PRICE * 1.1 AS '총합계'
FROM books
WHERE (TITLE LIKE '%공부%' OR TITLE LIKE '%설계%') AND (CATEGORY = 'IT');
