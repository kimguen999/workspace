...
# 30번 부서에 속한 사원의 어느 한명(최소로 받는 사람)보다도 급여를 더 많이 받는 사원의 사번, 사원명, 급여를 조회
# 어느 === ANY
SELECT
	EMPNO
	, ENAME
...
# 30번 부서에 속한 사원의 모두보다도 급여를 더 많이 받는 사원의 사번, 사원명, 급여를 조회
# 모두보다 === ALL
SELECT
	EMPNO
	, ENAME
...
# 3. '인천' 혹은 '서울' 지역에 근무하는 사원들의 성명, 직급을 조회
SELECT
	ENAME
	, JOB
FROM EMP
WHERE DEPTNO IN (SELECT DEPTNO 
				FROM DEPT 
				WHERE LOC = '인천' OR LOC = '서울');

# 4. 직급이 '과장'인 사원들이 받는 모든 급여보다 적게받는 직원들의 이름, 급여를 조회
SELECT
	ENAME
	, SAL
FROM EMP
